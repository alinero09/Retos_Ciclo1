...
def main():
    ...
    while n < 1:
        ...
        n = int(initial_data[0])
        m = int(initial_data[1])
        acum_type_ant = [] 
    ...
    for i in range(m):  
        ...
        if 0 < depart <= n:
            if type_new == 'a' :  
                acum_type_ant[depart - 1][convert_type_to_int(type_new)] += max(0, math.ceil((area - area_old * ant_old) / 11400))  # Aqui debes realizar el calculo de las antenas de tipo a con su rango respectivo
            elif type_new == 'b':
                acum_type_ant[depart - 1][convert_type_to_int(type_new)] += max(0, math.ceil((area - area_old * ant_old) / 12600))  # Aqui debes realizar el calculo de las antenas de tipo b con su rango respectivo
            elif type_new == 'c' :  
                acum_type_ant[depart - 1][convert_type_to_int(type_new)] += max(0, math.ceil((area - area_old * ant_old) / 41100))  # Aqui debes realizar el calculo de las antenas de tipo c con su rango respectivo
            elif type_new == 'd' :  
                acum_type_ant[depart - 1][convert_type_to_int(type_new)] += max(0, math.ceil((area - area_old * ant_old) / 14700))
            elif type_new == 'e':
                acum_type_ant[depart - 1][convert_type_to_int(type_new)] += max(0, math.ceil((area - area_old * ant_old) / 38000))  # Aqui debes realizar el calculo de las antenas de tipo e con su rango respectivo
